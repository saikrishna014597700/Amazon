...
let getOrdersBySellerName = async (msg, callback) => {
  let response = {};
  let err = {};
  const today = moment();
  ...
  try {
    ...
    const queryForSellerId = `SELECT id FROM amazonDB.users where name="${msg.searchTerm}" and role="Seller"`;
    query(pool, queryForSellerId)
      .then(async (result) => {
      ...
      .then((sellerId) => {
        console.log("1");
        let constructedQuery = `SELECT order_Id, GROUP_CONCAT(product_id) as product_id,GROUP_CONCAT(quantity) as quantity \
...
exports.getOrdersBySellerName = getOrdersBySellerName;
